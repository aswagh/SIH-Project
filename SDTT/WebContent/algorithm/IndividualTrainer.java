...
public class IndividualTrainer {
	 ...
	 */
	public IndividualTrainer(Timetable timetable) {
		
		int numTrainerDivMappings = timetable.getNumTrainerDivMappings();
		
		// 1 gene for trainer (number of subject trainer pairs)
		int chromosomeLength = numTrainerDivMappings;
		
		int newChromosome[] = new int[chromosomeLength];
		int chromosomeIndex = 0;
		
		//allocation of random professor to a div and its subject.
		for(String yyear:timetable.getYearArray()) {
			for(Group div:timetable.getDivArray(yyear)) {
				for(Module subs:timetable.getModuleArray(yyear)) {
					...
					newChromosome[chromosomeIndex] = proffId;
					chromosomeIndex++;			
				}
				for(LabModules labsubs:timetable.getLabModuleArray(yyear)) {
					int proffId = labsubs.getRandomProfessorId();
					newChromosome[chromosomeIndex] = proffId;
					chromosomeIndex++;
				}
			}
	 ...
	 */
	public IndividualTrainer(int chromosomeLength) {
	 ...
	 */
	public IndividualTrainer(int[] chromosome) {
