				...
				for(Module subs:timetable.getModuleArray(yyear)) {
					...
					if(sessionDuration == 1) {
						...
						for(int i=0;i<sessionsPerWeek;i++) {
							...
							newChromosome[chromosomeIndex] = slot;
							chromosomeIndex++;
							
							int hall = timetable.getRandomRoom();
							newChromosome[chromosomeIndex] = hall;
							chromosomeIndex++;
						}
					}
					else {
						...
						/*
						for(int i=0;i<sessionsPerWeek;i++) {
							int slots[] = timetable.getRandomLabTimeslot();
							timeslots.put(hashIdslots, slots);
							newChromosome[chromosomeIndex] = hashIdslots;
							chromosomeIndex++;
							hashIdslots++;
							
							int labs[] = timetable.getRandomLabsRoom();
							trainingHalls.put(hashIdrooms, labs);
							newChromosome[chromosomeIndex] = hashIdrooms;
							chromosomeIndex++;
							hashIdrooms++;
 						}
 						*/
