	...
	<table class="table m-5 ">
	  <thead class="black white-text">
		...
		<tbody>
			...
			%>
			<tr>
			<th class="bg-success border text-center h5"><%= rs.getString("day") %>
					<br><a class="btn btn-light pl-3 border" href="config/dynamicTT.jsp?day=<%= rs.getString("day")%>&trainee_id=<%= trainer_id%>" style="border-radius:15px;">Unavailable</a>
			</th>

								
			<% query="select * from timetable where day='"+rs.getString("day")+"' AND Professor_id="+trainer_id+"";
				...
				<% while(rs2.next()){
					...
					<a class="btn btn-light pl-3 border" href="config/dynamicSubTT.jsp?room=<%= rs2.getInt("room")%>&div=<%= rs2.getInt("div_id")%>&sub=<%= rs2.getInt("sub_id")%>&slot=<%= rs2.getString("timeslot")%>&status=s" style="border-radius:15px;">Unavailable</a>
